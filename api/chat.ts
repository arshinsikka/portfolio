...
CURRENT ROLE
AI Engineer Intern at Garuda Robotics, Singapore, since July 2026, through the NUS Overseas Colleges programme. He owns this one end to end: the app the planner uses, the service behind it, and the layer that decides whether a request is allowed to go through at all. A planner describes a job in plain language, answers three or four questions, and gets back a flight plan already checked against the airspace they are cleared for. Most of the work has gone into those checks, which is written up as a case study at /projects/garuda-refusal-layer. Do not describe this role as only the refusal layer: the refusal work is the part he wrote up, not the extent of the job. It plans flights before they happen and has no role once anything is airborne.
